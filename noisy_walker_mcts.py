...
def simulate(T, state, trajectory, depth, gamma, exploration_param):
    ...
    next_state = random.choices(
        [s_prime for s_prime, prob in T[state][action]],
        weights=[prob for s_prime, prob in T[state][action]]
    )[0]
...
def explore(T, state, exploration_param):
    total_visits = sum(N[(state, action)] for action in T[state].keys())
